    ...
    <?php $verificado = verificarPedidos(); ?>
        <div class="container backg">
            ...
            <div class="d-flex justify-content-center header">
                ...
                <button type="button" class="btn btn-primary btn-cliente" data-toggle="collapse" data-target="#collapse_producto" aria-expanded="false" aria-controls="collapse_producto" >Productos</button>
                <div class="collapse" id="collapse_producto">
                    <div class="card card-body nCliente">
                        <div class="form-group">
                            <form action="views/nuevo_producto.php" method="POST" id="nuevo_pedido">
                                <input value="Administrar Productos" class="btn btn-primary" style="width: -webkit-fill-available;margin-top: 15px;" type="submit">
                            </form>
                        </div>
                    </div>
                </div>

                <button type="button" class="btn btn-primary btn-cliente" data-toggle="collapse" data-target="#collapse_nCliente" aria-expanded="false" aria-controls="collapse_nCliente" >Clientes</button>
                <div class="collapse" id="collapse_nCliente">
                    <div class="card card-body nCliente">
                        ...
                        <a href="/views/editar_clientes.php" class="btn btn-warning d-flex justify-content-center" style="margin-top: 20px">Editar clientes</a>
                    </div>
                </div>
                ...
                <div class="collapse" id="collapse_historial">
                    <div class="card card-body nCliente">
                        <div class="form-group">
                            <form action="views/historial_pedido.php" method="POST" id="buscar_pedido">
                                <label for="Fecha">Fecha: </label>
                                <div id="datepicker" class="input-group date container col" data-date-format="dd-M-yyyy"> 
...
<?php
    ...
    function obtenerClientes(){
        ...
        if ($result = $conn -> query('SELECT id_cliente, nombre, completado FROM cliente WHERE usuario = "'.$usuario.'" ORDER BY nombre ASC')) {
    ...
    function verificarPedidos(){
        ...
        $pedidos_completados = mysqli_num_rows($conn -> query('SELECT id_cliente FROM cliente WHERE completado = 1 AND usuario = "'.$usuario.'"'));
        if ($result && $pedidos_completados > 0) {
            while($obj = $result->fetch_object()){
                if($obj->cantFecha > 0){
                    $existen = true;
                } else {
                    if($conn->query('UPDATE cliente SET completado = 0 WHERE usuario = "'.$usuario.'"')){
                        $existen = false;
                    }
                }
            }
